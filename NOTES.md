...
### 审计出问题后交给谁修？——审计只读 + 主对话修复（结合教程思想）
问：agent-config-audit 审出问题后，修复放哪合适？塞进这个 skill？新建 subagent？还是交给主对话？
答：结合教程原则选了「审计只读 + 主对话修复」。三条依据：①**审计与修复是两件事，别揉进一个 skill**——03-skills 说「一个 skill 聚焦一类、别做大杂烩」，还能用 `disallowed-tools` 收窄成只读；内置 `/code-review`（只审查）与 `/simplify`/`--fix`（才应用修复）本就分离。塞进去=违反单一职责，还让你不敢随手跑它。②**谁修看任务性质**——04-subagents 反对给「小/强耦合/需反复交互判断」的任务开 subagent；配置修复正是少而需判断、且有一整类 `[实践]` 项根本不是改文件（是改习惯）→ 交**主对话**逐条确认后改，审计结果本来就在对话上下文里、无需交接。③**别过早抽象**（07-plugins）——现在不建 fix skill/subagent；只有当修复变**批量/机械/可并行**（几十个文件同一处错）时才升级成独立修复能力，且审计与修复**永不合并**。收获：只读审计 + 人在环里修，是最省事也最贴 Claude Code 分工思想的做法。（2026-07-05）
## 04-subagents 子代理
## 05-mcp 外部集成
